...
Begin VB.Form FrmCNCSN 
   Caption         =   "PDM-CONCESSION Number Admin 工程管理子系统"
   ClientHeight    =   8160
   ClientLeft      =   60
   ClientTop       =   510
   ClientWidth     =   14025
...
'新功能模块中 所有的Call Refresh_CNCSN(lCurrentpage)中的CNCSN要统一置换为新表的名
Option Explicit
...
FrmCNCSNEdit.Show 1                                     '##########对应编辑窗口打开
Call Refresh_CNCSN(lCurrentpage) '添加完成后再刷新一次
End Sub
...
Dim TempCNCSNID As String                            '##########TempCNCSNID更换成对应表中Key字段名
'保存待删除记录的ID
  TempCNCSNID = objrs.Fields(0)                      '##########TempCNCSNID更换成对应表中Key字段名
...
'弹出删除确认对话框 Str是数字变字符串的函数,这里如果不用Str会出错
  If MsgBox("Confirm to delete" + objrs.Fields(0) + "?" + vbCrLf + "是否删除" + objrs.Fields(0) + "?", vbYesNo + vbDefaultButton2, "Confirm to Delete 确认删除") = vbYes Then
    ...
    '调用类中Delete函数删除CNCSN信息
    MyCNCSN.Delete (TempCNCSNID)                    '##########TempCNCSNID更换成对应表中Key字段名
...
'保存待修改记录的原始ID
FrmCNCSNEdit.OriCNCSNIndex = Trim(objrs.Fields(0))           '##########对应编辑窗口变量赋值

'把待修改信息添加到编辑窗口
FrmCNCSNEdit.TxtCNCSNIndex = Trim(objrs.Fields(0))           '##########对应编辑窗口控件赋值
FrmCNCSNEdit.TxtApplicant = Trim(objrs.Fields(1))             '##########对应编辑窗口控件赋值
FrmCNCSNEdit.TxtCPCNMP = Trim(objrs.Fields(2))                 '##########对应编辑窗口控件赋值
FrmCNCSNEdit.TxtDescription = Trim(objrs.Fields(3))            '##########对应编辑窗口控件赋值
FrmCNCSNEdit.TxtIDSO = Trim(objrs.Fields(4))                   '##########对应编辑窗口控件赋值
FrmCNCSNEdit.TxtOpnDate = Trim(objrs.Fields(5))                '##########对应编辑窗口控件赋值
FrmCNCSNEdit.TxtClosDate = Trim(objrs.Fields(6))               '##########对应编辑窗口控件赋值
FrmCNCSNEdit.TxtPJNOIndex = Trim(objrs.Fields(7))               '##########对应编辑窗口控件赋值
FrmCNCSNEdit.TxtPjtName = Trim(objrs.Fields(8))               '##########对应编辑窗口控件赋值
FrmCNCSNEdit.TxtFinsGdNO = Trim(objrs.Fields(9))               '##########对应编辑窗口控件赋值
FrmCNCSNEdit.TxtSglPrtNO = Trim(objrs.Fields(10))               '##########对应编辑窗口控件赋值
FrmCNCSNEdit.TxtCommtNote = Trim(objrs.Fields(11))               '##########对应编辑窗口控件赋值
...
rcds.Open "select * from CNCSN", Conn, adOpenKeyset, adOpenStatic  '启动一个Static类型的游标,否则记录数RecordCount总为-1 '##########对应表名字CNCSN
...
NextLine:
          ...
          For lCount = 1 To nPageSize   'nPageSize每页显示的记录数为10
                  ...
                  objrs.AddNew
                  objrs!CNCSNIndex = rcds!CNCSNIndex                                             '##########对应表字段赋值
