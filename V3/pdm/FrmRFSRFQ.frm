...
Begin VB.Form FrmRFSRFQ 
   Caption         =   "PDM-RFS/RFQ Number Admin 工程管理子系统"
   ClientHeight    =   7980
   ClientLeft      =   60
   ClientTop       =   510
   ClientWidth     =   11280
...
'新功能模块中 所有的Call Refresh_RFSRFQ(lCurrentpage)中的RFSRFQ要统一置换为新表的名
Option Explicit
...
FrmRFSRFQEdit.Show 1                                     '##########对应编辑窗口打开
Call Refresh_RFSRFQ(lCurrentpage) '添加完成后再刷新一次
End Sub
...
Dim TempRFSRFQID As String                            '##########TempRFSRFQID更换成对应表中Key字段名
'保存待删除记录的ID
  TempRFSRFQID = objrs.Fields(0)                      '##########TempRFSRFQID更换成对应表中Key字段名
...
'弹出删除确认对话框 Str是数字变字符串的函数,这里如果不用Str会出错
  If MsgBox("Confirm to delete" + Str(objrs.Fields(0)) + "?" + vbCrLf + "是否删除" + Str(objrs.Fields(0)) + "?", vbYesNo + vbDefaultButton2, "Confirm to Delete 确认删除") = vbYes Then
    ...
    MyRFSRFQ.Delete (TempRFSRFQID)                    '##########TempRFSRFQID更换成对应表中Key字段名
...
'保存待修改记录的原始ID
FrmRFSRFQEdit.OriRFSRFQIndex = Trim(objrs.Fields(0))           '##########对应编辑窗口变量赋值

'把待修改信息添加到编辑窗口
FrmRFSRFQEdit.TxtRFSRFQIndex = Trim(objrs.Fields(0))           '##########对应编辑窗口控件赋值
FrmRFSRFQEdit.TxtApplicant = Trim(objrs.Fields(1))             '##########对应编辑窗口控件赋值
FrmRFSRFQEdit.TxtLeader = Trim(objrs.Fields(2))                 '##########对应编辑窗口控件赋值
FrmRFSRFQEdit.TxtDescription = Trim(objrs.Fields(3))            '##########对应编辑窗口控件赋值
FrmRFSRFQEdit.TxtIDSQ = Trim(objrs.Fields(4))                   '##########对应编辑窗口控件赋值
FrmRFSRFQEdit.TxtOpnDate = Trim(objrs.Fields(5))                '##########对应编辑窗口控件赋值
FrmRFSRFQEdit.TxtClosDate = Trim(objrs.Fields(6))               '##########对应编辑窗口控件赋值
...
rcds.Open "select * from RFSRFQ", Conn, adOpenKeyset, adOpenStatic  '启动一个Static类型的游标,否则记录数RecordCount总为-1  '##########对应表名字RFSRFQ
...
NextLine:
          ...
          For lCount = 1 To nPageSize   'nPageSize每页显示的记录数为10
                  ...
                  objrs.AddNew
                  objrs!RFSRFQIndex = rcds!RFSRFQIndex                                             '##########对应表字段赋值
