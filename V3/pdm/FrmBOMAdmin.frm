...
Private Sub CmdBuildFirstBOM_Click()
    ...
    ItemNumber = left(Text1.Text, 4)
    'MsgBox ItemNumber
    
    'MsgBox (ItemNumber <> 9999)
    ...
    ElseIf ItemNumber <> 2441 And ItemNumber <> 9041 And ItemNumber <> 9999 And ItemNumber <> 4341 Then
    
    'ElseIf (left(Text1.Text, 4) <> "9999") Then
...
Private Sub Command1_Click()
    ...
    ElseIf Not (Len(Trim(Text1.Text)) = 12 And Isnum(Trim(Text1.Text))) Then
        MsgBox "Finish Goods is 12 Number, no Letterssss" + vbCrLf + "必须是12位数字的编号,无字母", vbInformation, "System Info."
        Me.Enabled = True
        frameMsg.Visible = False
        Exit Sub
    Else
    ...
    If LCase(temp_tb_SglPrt4BOMLog) <> "sglprt4bomlog" Then
...
Private Sub DropTempTable()
    ...
    If FinishGoodsNO <> "" Then
        If temp_tb_BOMOrigData <> "" And temp_tb_BOMOrigData <> "BOMOrigData" Then
            StrSql = "IF EXISTS (select top 1 * from " & temp_tb_BOMOrigData & ") DROP TABLE " & temp_tb_BOMOrigData
            Conn.Execute StrSql
        End If
        
        If temp_tb_SglPrt4BOMLog <> "" And temp_tb_SglPrt4BOMLog <> "SglPrt4BOMLog" Then
            StrSql = "IF EXISTS (select top 1 * from " & temp_tb_SglPrt4BOMLog & ") DROP TABLE " & temp_tb_SglPrt4BOMLog
            Conn.Execute StrSql
        End If
    End If
    If Err <> 0 Then
