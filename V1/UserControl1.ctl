...
'we use this to open a database and start our commands like Delete/Add new /Find...
...
Public Function FindRecord(ColName As String, ByVal FindWay As Way, Text As String) As String
On Error GoTo BestHandler
Dim I As Integer
Dim J As Integer
...
'you know, its the certeria used in sql, we use [] if we have column caption from two words like (Full Name), if you don't use the [] in this case, error will occured
 RecordsetT.Find "[" & ColName & "]" & " ='" & Text & "'", 0, adSearchForward  '后面加,1的话表示从第1个记录开始找

Else
 RecordsetT.Find "[" & ColName & "]" & " Like '" & Text & "%'", 0, adSearchForward '后面加,1的话表示从第1个记录开始找
...
End If
    ...
    I = RecordsetT.Fields.Count  '获取字段总数做循环
      For J = 0 To I - 1
       UsrCtlFind(J) = RecordsetT.Fields(J)     'UsrCtlFind是全局整体模块的一个数组变量
      Next
