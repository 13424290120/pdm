...
Private Sub cmdBOMSave_Click()
    ...
    With MSFlexGrid1
        For i = 2 To .Rows - 2
            If Trim(.TextMatrix(i, 2)) <> "" Then
                ...
                StrSql = StrSql & "Insert into  " & temp_tb_SglPrt4BOMLog & " (BOM,SeqIndex,ParentID,ChildID,BOMVersion,Quantity,PrtUnit,Description,ItemType,SERNmbr,ChgStatus,CommtNote,Family) Values("
    ...
    If Err Then
        Conn.RollbackTrans
        MsgBox "Save BOM Version Failed, Please contact System Admin.", vbInformation, "System Info"
    Else
        Call UpdateBOMVerQtyDesc(FinishGoodsNO, CStr(CurVersion), MSFlexGrid1)
        Me.Enabled = True
        frameMsg.Visible = False
        '##########询问用户要不要解锁BOM#######
        If MsgBox("Save BOM Version Successfully. Would you like to unlock the BOM?", vbYesNo, "System Info") = vbYes Then
        ...
        Conn.CommitTrans
        If CurVersion <> 1 Then Call SaveBOMData
        bNotSave1stVer = True
...
Private Sub GeneralPathAdd(ByVal InputPathName As String, ByVal InputField As String)
    ...
    If DocPathName = "" Then
        ...
        Exit Sub
    ElseIf Not (Mid(DocPathName, 1, 3) = "X:\" Or Mid(DocPathName, 1, 3) = "U:\" Or Mid(DocPathName, 1, 3) = "Z:\") Then
        MsgBox "The Document Path must be formal Released or Shared(X:\,U:\,Z:\)", vbInformation, "System Info."
        Exit Sub
...
Private Sub LblCPCNPathSeek_Click()
    ...
    If Len(Dir("D:\Temp\Search.txt")) > 0 Then Kill "D:\Temp\Search.txt"  '判断一个文件是否存在,存在则删除
    Filename = Trim(CPCN)
    'FileName = Mid(FileName, 1, Len(FileName) - 1) '去掉最右边一个字符
    Filename = "*" & Filename & "*"  '前后加星号
    GetFilePath = InputBox("Please input Directory Path", "Prompt Info 输入搜索路径", "X:\CP-CN\", 10000, 1)    'InputBox (Message, Title, Default,其中10000, 1是窗口出现位置)
    
    'Shell "CMD /K " & Left(GetFilePath, 2) '首先要进到对应的驱动器盘符下,即左取2位字符
    'Shell "CMD /K CD " & GetFilePath       '再进到对应目录下
    'Shell "CMD /C DIR " & FileName & " /A/L/B/S >D:\Temp\Search.txt", 1 '(/L小写字母,/S包括子目录,/B是没有headingTitle和Summary /A显示特别属性文件  1 VbNormalFocus 窗口具有焦点,且会还原到原来的大小位置 )
    
    If Len(Dir("D:\Temp\Share.bat")) > 0 Then                    '查看批处理文件存在否
        Kill "D:\Temp\Share.bat"                                 '存在则删除
    End If
    Open "D:\Temp\Share.bat" For Output As #2                '打开文件准备写入
    Print #2, left(GetFilePath, 2)
    Print #2, "CD " & GetFilePath
    Print #2, "DIR " & Filename & " /A/B/S >D:\Temp\Search.txt"
    Close #2
    ...
    Open "D:\Temp\Search.txt" For Input As #1
    ...
    Else
        ...
        OpnShllExcFile ("D:\Temp\Search.txt")
    End If
...
Private Sub LblSERPathSeek_Click()
    ...
    Filename = Mid(Filename, 1, Len(Filename) - 1) '去掉最右边一个字符
    Filename = "*" & Filename & "*"  '前后加星号
    GetFilePath = InputBox("Please input Directory Path", "Prompt Info 输入搜索路径", "X:\SER\", 10000, 1)    'InputBox (Message, Title, Default,其中10000, 1是窗口出现位置)
...
Private Sub LblDRWPathSeek_Click()
    ...
    Filename = "*" & Filename & "*"  '前后加星号
    GetFilePath = InputBox("Please input Directory Path", "Prompt Info 输入搜索路径", "X:\DRAWING\", 10000, 1)    'InputBox (Message, Title, Default,其中10000, 1是窗口出现位置)
...
Private Sub Command1_Click()
    ...
    Refresh_FlexGrid_TreeView True
        ...
        Do While Not rs.EOF
            ...
            If rs2.RecordCount > 0 Then
                ...
                If Not IsNull(rs2.Fields("SERLocate")) Then
                    TempSER = Mid(Replace(Trim(rs2.Fields("SERLocate")), "----", ""), 16, 4)
...
Private Sub InsertBOMLog4Add(ByVal Qty As String, ByVal SglPrtKey As String, ByVal ParentKey As String, ByVal rowIndex As Integer, ByVal GrandpaKey As String, ByVal xNode As Node)
    ...
    If rs.RecordCount > 0 Then
        ...
        If Not IsNull(rs.Fields("SERLocate")) Then
            TempSER = Mid(Replace(Trim(rs.Fields("SERLocate")), "----", ""), 16, 4)
            If TempSER = "EASE" Then
                TempSER = "RELEASREPORT"
            Else
                TempSER = "SER00000" & TempSER
...
Public Sub Get_BOM_FlexGrid(ByVal parentid As String, ByVal TempLevel As Integer)
    ...
    If objrs.RecordCount > 0 Then
        Do While Not objrs.EOF
            ...
            If SERLocation <> "" Then
                TempSER = Mid(Replace(SERLocation, "----", ""), 16, 5)
                If TempSER = "EASE " Then
                    TempSER = "RELEASREPORT"
                Else
            ...
            If SERNumber = "" Then  '必须用IsNull函数判断,不能用 objrs3.Fields("SERNmbr") = Null
                If TempSER <> "" Then
    ...
    Set objConn = Nothing
End Sub
Public Sub Refresh_FlexGrid_TreeView(bBox As Boolean)
    On Error Resume Next
    ...
    If rstCX3.RecordCount > 0 Then
        ...
        If Not IsNull(rstCX3.Fields("SERLocate")) Then
            TempSER = Mid(Replace(rstCX3.Fields("SERLocate"), "----", ""), 16, 5)
            If TempSER = "EASE " Then
                TempSER = "RELEASREPORT"
            Else
                If right(TempSER, 1) = "-" Or right(TempSER, 1) = "(" Or right(TempSER, 1) = "（" Then
...
AdminGoAhead1:
        ...
        If CurVersion = 1 And MSFlexGrid1.Col = 4 Then
            Call checkModifyPermission(MSFlexGrid1.TextMatrix(MSFlexGrid1.Row, 2))
        End If
...
Private Function CheckIsApproved(ByVal FinsGdNO As String) As Boolean
    ...
    StrSql = "Select Approver,ApproveDate From BOMSubmitApprove  Where FinsGdIndex=" & FinsGdNO & " And (ApproveDate<>'' OR ApproveDate<>NULL) And Approver<>''"
    rs.Open StrSql, Conn, adOpenKeyset, adLockPessimistic
    If rs.RecordCount > 0 Then
        If rs.Fields("Approver") <> "" Or rs.Fields("Approver") <> Null Then
            If rs.Fields("ApproveDate") <> "" Or rs.Fields("ApproveDate") <> Null Then
    ...
    With MF
        For i = 2 To .Rows - 1
            If .TextMatrix(i, 2) <> "" And .TextMatrix(i, 3) <> "" Then
                StrSql = "UPDATE  " & temp_tb_SglPrt4BOMLog & "  SET Quantity=" & (.TextMatrix(i, 4)) & ", Description='" & (.TextMatrix(i, 6)) & "' WHERE BOM=" & bom & " AND BOMVersion=" & version & " AND ParentID='" & (.TextMatrix(i, 2)) & "' AND ChildID='" & (.TextMatrix(i, 3)) & "'"
                Conn.Execute StrSql
            End If
        Next
    End With
...
Private Sub SaveBOMData()
    ...
    '############更新用量修改#############
    StrSql = "SELECT ParentID,ChildID,Quantity FROM " & temp_tb_SglPrt4BOMLog & " WHERE ChgStatus = 'Modify'"
    rs.Open StrSql, Conn, adOpenKeyset, adLockOptimistic
    If rs.RecordCount > 0 Then
        Do While Not rs.EOF
            StrSql = "UPDATE SglPrt4BOMLog SET Quantity=" & rs.Fields("Quantity") & " WHERE ParentID=" & rs.Fields("ParentID") & " AND ChildID=" & rs.Fields("ChildID")
            Conn.Execute StrSql
            '################删除重复的记录################
            StrSql = "DELETE FROM SglPrt4BOMLog WHERE ParentID=" & rs.Fields("ParentID") & " AND ChildID=" & rs.Fields("ChildID") & " AND (ChgStatus = NULL OR ChgStatus = '')"
            Conn.Execute StrSql
        rs.MoveNext
        Loop
    End If
    ...
    '###########5.删除临时表###############
    ''Call DropTempTable  ''不可删除，删除就不能再编辑了
    ...
    If rs.RecordCount > 0 Then
        Do While Not rs.EOF
            rs2.Open "SELECT SglPrtVer FROM sglprt WHERE SglPrtIndex='" & left(rs.Fields("ChildID"), 11) & "0'", Conn, adOpenKeyset, adLockOptimistic
            Conn.Execute "UPDATE BOMOrigData SET ParentID = '" & left(rs.Fields("ChildID"), 11) & CStr(rs2.Fields("SglPrtVer")) & "' WHERE LEFT(ParentID,11)='" & left(rs.Fields("ChildID"), 11) & "'"
            Conn.Execute "UPDATE BOMOrigData SET ChildID = '" & left(rs.Fields("ChildID"), 11) & CStr(rs2.Fields("SglPrtVer")) & "' WHERE LEFT(ChildID,11)='" & left(rs.Fields("ChildID"), 11) & "'"
            rs2.Close
        rs.MoveNext
        Loop
    End If
    ...
    Else
        Conn.RollbackTrans
        MsgBox "BOM save failure, please try again.", vbCritical, "PDM Database"
    End If
